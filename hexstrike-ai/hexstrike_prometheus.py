...
class HexStrikeClient:
    ...
    def execute_command(self, command: str, use_cache: bool = True) -> Dict[str, Any]:
        """
        Execute a generic command on the HexStrike server asynchronously
        ...
        Returns:
            Command execution results with task_id for async tracking
        """
        return self.safe_post("api/process/execute-async", {
            "command": command,
...
def setup_mcp_server(hexstrike_client: HexStrikeClient) -> FastMCP:
    ...
    # ============================================================================
    # ADVANCED PROCESS MANAGEMENT TOOLS (v10.0 ENHANCEMENT)
    # ============================================================================

    @mcp.tool()
    def execute_command(command: str, use_cache: bool = True) -> Dict[str, Any]:
        """
        Execute an arbitrary command on the HexStrike AI server asynchronously.
        ...
        Returns:
            Task submission results with task_id for tracking async execution
        """
        try:
            logger.info(f"⚡ Submitting async command: {command}")
            result = hexstrike_client.execute_command(command, use_cache)
            
            if "error" in result:
                logger.error(f"❌ Command submission failed: {result['error']}")
                return {
                    "success": False,
                    "error": result["error"],
                    "task_id": None,
            ...
            if result.get("success"):
                ...
                logger.info(f"✅ Command submitted successfully | Task ID: {task_id}")
                logger.info(f"📋 Status: {result.get('status', 'submitted')}")
            else:
                logger.warning(f"⚠️  Command submission completed with warnings")

            return result
        except Exception as e:
            logger.error(f"💥 Error submitting command '{command}': {str(e)}")
            return {
                "success": False,
                "error": str(e),
                "task_id": None,
    ...
    @mcp.tool()
    def get_async_task_result(task_id: str) -> Dict[str, Any]:
        """
        Get result of asynchronous task execution.

        Args:
            task_id: The task ID to retrieve results for

        Returns:
            Task result including status, output, and execution details
        """
        logger.info(f"📋 Retrieving task result for: {task_id}")
        result = hexstrike_client.safe_get(f"api/process/get-task-result/{task_id}")
        
        if result.get("success"):
            status = result.get("result", {}).get("status", "unknown")
            logger.info(f"✅ Task result retrieved | Task ID: {task_id} | Status: {status}")
        else:
            logger.error(f"❌ Failed to retrieve task result for {task_id}")
        
        return result

    @mcp.tool()
    def get_process_pool_stats() -> Dict[str, Any]:
        """
        Get process pool statistics and performance metrics.

        Returns:
            Comprehensive statistics about process pool, active workers, queue size, and performance
        """
        logger.info("📊 Retrieving process pool statistics")
        result = hexstrike_client.safe_get("api/process/pool-stats")
        
        if result.get("success"):
            stats = result.get("stats", {})
            pool_info = stats.get("process_pool", {})
            active_workers = pool_info.get("active_workers", 0)
            logger.info(f"✅ Pool stats retrieved | Active workers: {active_workers}")
        else:
            logger.error("❌ Failed to retrieve pool stats")
    ...
    def get_cache_stats() -> Dict[str, Any]:
        """
        Get advanced cache statistics including hit rate, size, and performance.

        Returns:
            Cache statistics with hit rate, miss rate, total size, and entries
        """
        logger.info("💾 Retrieving cache statistics")
        result = hexstrike_client.safe_get("api/process/cache-stats")
        
        if result.get("success"):
            cache_stats = result.get("cache_stats", {})
            hit_rate = cache_stats.get("hit_rate", 0)
            logger.info(f"✅ Cache stats retrieved | Hit rate: {hit_rate:.1f}%")
        else:
    ...
    @mcp.tool()
    def clear_process_cache() -> Dict[str, Any]:
        """
        Clear the advanced process cache to free memory and reset statistics.

        Returns:
            Success status of cache clear operation
        """
        logger.info("🧹 Clearing process cache")
        result = hexstrike_client.safe_post("api/process/clear-cache", {})
    ...
    def get_resource_usage() -> Dict[str, Any]:
        ...
        Returns:
            Current CPU, memory, disk usage and historical trends
        """
        logger.info("📈 Retrieving resource usage")
        result = hexstrike_client.safe_get("api/process/resource-usage")
        
        if result.get("success"):
            usage = result.get("current_usage", {})
            cpu_percent = usage.get("cpu_percent", 0)
            memory_percent = usage.get("memory_percent", 0)
            logger.info(f"✅ Resource usage retrieved | CPU: {cpu_percent:.1f}% | Memory: {memory_percent:.1f}%")
        else:
    ...
    def get_performance_dashboard() -> Dict[str, Any]:
        """
        Get comprehensive performance dashboard with system health, pool stats, and resource usage.

        Returns:
            Complete dashboard with performance summary, process pool status, resource metrics, 
            cache stats, auto-scaling status, and system health indicators
        """
        logger.info("📊 Retrieving performance dashboard")
        result = hexstrike_client.safe_get("api/process/performance-dashboard")
        
        if result.get("success"):
            dashboard = result.get("dashboard", {})
            perf_summary = dashboard.get("performance_summary", {})
            success_rate = perf_summary.get("success_rate", 0)
            logger.info(f"✅ Dashboard retrieved | Success rate: {success_rate:.1f}%")
            
            # Log system health summary
            health = dashboard.get("system_health", {})
            logger.info(f"   ├─ CPU: {health.get('cpu_status', 'unknown')}")
            logger.info(f"   ├─ Memory: {health.get('memory_status', 'unknown')}")
            logger.info(f"   └─ Disk: {health.get('disk_status', 'unknown')}")
        else:
            logger.error("❌ Failed to retrieve performance dashboard")
        
        return result

    @mcp.tool()
    def terminate_process_gracefully(pid: int, timeout: int = 30) -> Dict[str, Any]:
        """
        Terminate a process with graceful degradation and configurable timeout.

        Args:
            pid: Process ID to terminate
            timeout: Timeout in seconds for graceful termination (default: 30)

        Returns:
            Success status of the graceful termination operation
        """
        logger.info(f"🛑 Gracefully terminating process {pid} | Timeout: {timeout}s")
        result = hexstrike_client.safe_post(f"api/process/terminate-gracefully/{pid}", {
            "timeout": timeout
        })
    ...
    @mcp.tool()
    def configure_auto_scaling(enabled: bool = True, cpu_threshold: float = None, 
                              memory_threshold: float = None, queue_threshold: int = None) -> Dict[str, Any]:
        """
        Configure auto-scaling settings for the process pool.

        Args:
            enabled: Enable or disable auto-scaling (default: True)
            cpu_threshold: CPU usage threshold for scaling (0-100, optional)
            memory_threshold: Memory usage threshold for scaling (0-100, optional)
            queue_threshold: Queue size threshold for scaling (optional)

        Returns:
            Auto-scaling configuration status and current thresholds
        """
        logger.info(f"⚙️ Configuring auto-scaling | Enabled: {enabled}")
        ...
        if queue_threshold is not None:
            thresholds["queue_threshold"] = queue_threshold
        
        result = hexstrike_client.safe_post("api/process/auto-scaling", {
            "enabled": enabled,
            "thresholds": thresholds
        })
        
        if result.get("success"):
            logger.info(f"✅ Auto-scaling configured successfully")
            if thresholds:
                logger.info(f"   └─ Thresholds updated: {thresholds}")
        else:
            logger.error("❌ Failed to configure auto-scaling")
        
        return result

    @mcp.tool()
    def scale_process_pool(action: str, count: int = 1) -> Dict[str, Any]:
        """
        Manually scale the process pool up or down.

        Args:
            action: Scaling action - "up" to add workers, "down" to remove workers
            count: Number of workers to add or remove (default: 1)

        Returns:
            Scaling operation result with previous and current worker counts
        """
        logger.info(f"📏 Manual scaling | Action: {action} | Count: {count}")
        
        if action not in ["up", "down"]:
            logger.error("❌ Invalid action: must be 'up' or 'down'")
            return {
                "success": False,
                "error": "Action must be 'up' or 'down'"
            }
        
        result = hexstrike_client.safe_post("api/process/scale-pool", {
            "action": action,
            "count": count
        })
        
        if result.get("success"):
            prev_workers = result.get("previous_workers", 0)
            curr_workers = result.get("current_workers", 0)
            logger.info(f"✅ Pool scaled successfully | Workers: {prev_workers} → {curr_workers}")
        else:
            logger.error(f"❌ Failed to scale pool: {result.get('error', 'Unknown error')}")
        
        return result

    @mcp.tool()
    def process_health_check() -> Dict[str, Any]:
        """
        Comprehensive health check of the process management system.

        Returns:
            Health report with overall status, health score (0-100), identified issues,
            system statistics, and recommendations for optimization
        """
        logger.info("🏥 Performing process management health check")
        result = hexstrike_client.safe_get("api/process/health-check")
        ...
        if result.get("success"):
            ...
            logger.info(f"✅ Health check completed | Status: {status} | Score: {score}/100")
